...
fun ICorChainDsl<MkpContext>.stubUpdateSuccess(title: String, corSettings: MkpCorSettings) = worker {
    this.title = title
    this.description = """
        Кейс успеха для изменения заказов
    """.trimIndent()
    ...
    handle {
        logger.doWithLogging(id = this.requestId.asString(), LogLevel.DEBUG) {
            ...
            val stub = MkpOpStub.prepareResult {
                opRequest.id.takeIf { it != MkpOpId.NONE}?.also { this.id = it }
